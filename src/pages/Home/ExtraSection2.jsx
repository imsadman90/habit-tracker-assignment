        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center  p-10 rounded-xl">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <div
                key={index}
                className="bg-gray-50 dark:bg-base-200 p-6 rounded-lg shadow-md hover:shadow-xl transition"
              >
                <div className="flex items-center gap-2 justify-center">
                  <div className="">{feature.icon}</div>
                  <h3 className="text-xl font-semibold">{feature.title}</h3>
                </div>
                <p className="text-gray-600 mt-4 dark:text-gray-300">{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
